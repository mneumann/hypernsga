// GUI using conrod instead of imgui
...
fn main() {
    ...
    let window: PistonWindow = WindowSettings::new("EvoGraph", [600, 400])
        .exit_on_esc(true).build().unwrap();

    let mut ui = {
        let theme = Theme::default();
        let glyph_cache = Glyphs::new("examples/fonts/NotoSans/NotoSans-Regular.ttf", window.factory.borrow().clone());
    ...
    for event in window.ups(30) {
        ...
        event.update(|_| ui.set_widgets(|ref mut ui| {
            ...
            conrod::Button::new()
                .middle_of(CANVAS)
                .w_h(80.0, 80.0)
                .label(&format!("{}", state.iteration))
            .react(|| {state.running = !state.running;})
            .set(COUNTER, ui);
        }));
