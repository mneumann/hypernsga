...
fn write_net_gml<P>(filename: &str,
                expression: &Expression,
                genome: &G,
                substrate_config: &SubstrateConfiguration<P, Neuron>)
    where P: Position
...
fn write_net_dot<P>(filename: &str,
                expression: &Expression,
                genome: &G,
                substrate_config: &SubstrateConfiguration<P, Neuron>)
    where P: Position
...
fn write_cppn_dot(filename: &str, genome: &G)
{
    let mut file = File::create(filename).unwrap();
    ...
    loop {
        {
            ...
            match state.action {
                Action::ExportBest => {
                    ...
                    write_net_gml(&format!("{}.gml", basefilename), &expression, best.genome(), &substrate_config);
                    write_net_dot(&format!("{}.dot", basefilename), &expression, best.genome(), &substrate_config);
                    write_cppn_dot(&format!("{}.cppn.dot", basefilename), best.genome());
                }
                Action::ResetNet => {
                    parents = {
                        let mut initial = UnratedPopulation::new();
